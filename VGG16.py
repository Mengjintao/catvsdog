# -*- coding: utf-8 -*-
"""
Created on Fri Mar  1 16:33:18 2019
...
def VGG16(images, _dropout, n_cls):
    """
        ...
        全连接层使用xavier类型初始化
    """
    conv1_1 = conv(images, 64, 'conv1_1', fineturn=False)
    conv1_2 = conv(conv1_1, 64, 'conv1_2', fineturn=False)
    pool1   = maxpool(conv1_2, 'pool1')
 
    conv2_1 = conv(pool1, 128, 'conv2_1', fineturn=False)
    conv2_2 = conv(conv2_1, 128, 'conv2_2', fineturn=False)
    pool2   = maxpool(conv2_2, 'pool2')
 
    conv3_1 = conv(pool2, 256, 'conv3_1', fineturn=False)
    conv3_2 = conv(conv3_1, 256, 'conv3_2', fineturn=False)
    conv3_3 = conv(conv3_2, 256, 'conv3_3', fineturn=False)
    pool3   = maxpool(conv3_3, 'pool3')
 
    conv4_1 = conv(pool3, 512, 'conv4_1', fineturn=False)
    conv4_2 = conv(conv4_1, 512, 'conv4_2', fineturn=False)
    conv4_3 = conv(conv4_2, 512, 'conv4_3', fineturn=False)
    pool4   = maxpool(conv4_3, 'pool4')
 
    conv5_1 = conv(pool4, 512, 'conv5_1', fineturn=False)
    conv5_2 = conv(conv5_1, 512, 'conv5_2', fineturn=False)
    conv5_3 = conv(conv5_2, 512, 'conv5_3', fineturn=False)
